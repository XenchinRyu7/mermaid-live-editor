<script lang="ts">
  ...
  const onExportPNG = () => {
    if (!$urlsStore.png) {
      notify('Unable to export PNG. Please try again after the diagram finishes rendering.');
      return;
    }
  ...
  const onExportSVG = () => {
    if (!$urlsStore.svg) {
      notify('Unable to export SVG. Please try again after the diagram finishes rendering.');
      return;
    }
